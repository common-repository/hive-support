...
function hivesupport_htmlToPlainText($str)
{
    $str = str_replace('&nbsp;', ' ', $str);
...
if (current_user_can('hive_support_access_plugin')) {
    ...
    $contenthash = md5("Replay Hive Support");
    ...
    $ticket_data['agent_id'] = $personid;
    $ticket_data['response_text'] = $replaymessage;

    do_action('hs_ticket_aresponse_added', $ticket_data);
    ...
    $wpdb->update(
        ...
        array(
            ...
            'customer_seen_status' => 'new_reply'
        ),
        array( 'id' => absint( $ticketid ) )
    );

    $ac_return = HiveSupportUtils::add_activities("New Replay added", $personid, "Administrator", $ticketid);
